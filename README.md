...
- 카드의 D-day 영역이나 메뉴 막대의 **D-day 설정 / 변경…**에서 행사 이름과 목표 날짜·시각을 정합니다. `D-2`와 `50:02:30` 형태로 남은 전체 시간을 매초 표시합니다. 24시간 미만이면 `D-DAY`로 표시합니다. **D-day 지우기**로 설정을 없앨 수 있습니다.
- 카드의 연필 아이콘이나 메뉴 막대의 **문구 추가…**에서 원하는 문구를 여러 개 추가할 수 있습니다. 새 문구는 즉시 표시되고 이후 경배할 때 기본 문구와 함께 돌아갑니다. **사용자 문구 삭제…**에서 하나씩 지울 수 있습니다. 문구는 이 Mac에만 저장됩니다.
